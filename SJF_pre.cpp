...
void process_scheduler(){
	...
	while(1){
		...
		while((sm[0].pid)!=done){
			//cout<<"done "<<done<<endl;;
			int sch=-1,minb=1000000000;
			for(int i=1;i<sm[0].pid;i++){
				if((sm[i].burst>0) && (sm[i].burst<minb)){
					minb = sm[i].burst;
					sch = i;
				}
			}
			...
			if(prevsch!=sch){
				if(prevsch!=-1){
					if(sm[prevsch].burst>0){
						printf("Time: %d Process %d with remaining burst time %d is pre-empted.\n",pret,prevsch,sm[prevsch].burst);
					}
				}
				printf("Time: %d Process %d with burst time %d starts.\n",clock(),sch,minb);
			}
			prevsch=sch;
			//sm[sch].val=0;
